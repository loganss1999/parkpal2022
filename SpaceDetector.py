...
def findSpace(nusc, sample, xL, yL, xR, yR, orientation):
    points = [(xL,yL),(xR,yR)]
    ...
    cars.sort()
    spaces = []
#     space = defineSpace(xL,yL,cars[0][0],cars[0][1],0,cars[0][2])
#     if(space[0] > 2.5):
#         spaces.append(space)
    i = 0
    l = len(cars)
    while i < l-1:
        space = defineSpace(cars[i][0],cars[i][1],cars[i+1][0],cars[i+1][1],cars[i][2],cars[i+1][2])
        if(space[0] > 2.5):
            spaces.append(space)
        i = i + 1
#     space = defineSpace(cars[i][0],cars[i][1],xR,yR,cars[0][2],0)
#     if(space[0] > 2.5):
#             spaces.append(space)
